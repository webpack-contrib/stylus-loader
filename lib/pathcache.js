...
// webpack's resolver.
function PathCache(contexts, sources) {
...
PathCache.create = function(contexts, sources) {
  return when(new PathCache(contexts, sources));
};
...
function resolveFileDeep(helpers, parentCache, source, fullPath) {
  ...
  return when
    ...
    .then(function() {
      return PathCache.create(contexts, sources);
    });
}
